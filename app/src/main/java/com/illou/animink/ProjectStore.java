...
final class ProjectStore {
    ...
    static AnimationProject loadRecovery(Context context) {
        ...
        if (!source.exists()) {
            File legacy = new File(context.getFilesDir(), "autosave.aink");
    ...
    static AnimationProject load(InputStream source) throws IOException {
        try (DataInputStream input = new DataInputStream(new BufferedInputStream(source))) {
            if (input.readInt() != MAGIC) throw new IOException("Format AnimInk non reconnu");
            ...
            throw new IOException("Version AnimInk non reconnue");
        }
    }
        ...
        try {
            for (int frameIndex = 0; frameIndex < frameCount; frameIndex++) {
                ...
                for (int tileIndex = 0; tileIndex < tileCount; tileIndex++) {
                    ...
                    if (input.readInt() != LEGACY_TILE_BYTES) throw new IOException("Tuile AnimInk v2 invalide");
            ...
            for (int layerIndex = 0; layerIndex < AnimationProject.LAYER_COUNT; layerIndex++) {
                ...
                for (int tileIndex = 0; tileIndex < tileCount; tileIndex++) {
                    ...
                    if (input.readInt() != AnimationProject.TILE_BYTES)
                        throw new IOException("Tuile AnimInk v3 invalide");
                    input.readFully(tileBytes);
    ...
    private static AtomicFile recoveryFile(Context context) {
        return new AtomicFile(new File(context.getFilesDir(), "recovery.aink"));
    }

    private static int checked(int value, int max) throws IOException {
        if (value < 0 || value > max) throw new IOException("Taille de projet invalide");
        return value;
    }

    private static int checkedRange(int value, int min, int max) throws IOException {
        if (value < min || value > max) throw new IOException("Dimension de projet invalide");
        return value;
    }
