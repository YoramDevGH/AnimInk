...
/** Native AnimInk workspace hosted inside the Supernote React Native plugin view. */
public final class AnimInkPluginView extends LinearLayout implements InkCanvasView.Listener,
        ActivityEventListener, LifecycleEventListener {
    private static final int REQUEST_OPEN = 7320;
    private static final int REQUEST_CREATE = 7321;
    ...
    public AnimInkPluginView(ReactContext context) {
        super(context);
    ...
    private void initialize() {
        ...
        session = getContext().getSharedPreferences("animink_plugin_session",
                android.content.Context.MODE_PRIVATE);
        project = ProjectStore.loadRecovery(getContext());
        projectName = session.getString("name", "Untitled");
    ...
    private void buildInterface() {
        ...
        Button fileButton = textButton("File", v -> showFileMenu(v));
        ...
        TextView gestureHint = new TextView(getContext());
        ...
        // Reserved for the React Native "Close" button overlaid by PluginHost.
        header.addView(new View(getContext()),
                new LinearLayout.LayoutParams(dp(96), ViewGroup.LayoutParams.MATCH_PARENT));
        ...
        LinearLayout sketchRow = layerRow("Sketch", AnimationProject.LAYER_SKETCH);
        ...
        canvas = new InkCanvasView(getContext());
        ...
        HorizontalScrollView timelineScroll = new HorizontalScrollView(getContext());
        ...
        addView(root, new LinearLayout.LayoutParams(
    ...
    private LinearLayout layerRow(String label, int layer) {
        ...
        visibility.setContentDescription("Show or hide the " + label + " layer");
    ...
    private void showFileMenu(View anchor) {
        ...
        menu.getMenu().add(0, 1, 0, "New");
        menu.getMenu().add(0, 2, 1, "Open…");
        menu.getMenu().add(0, 3, 2, "Save");
    ...
    private void confirmDiscardThen(Runnable action) {
        ...
        new AlertDialog.Builder(currentActivity() != null ? currentActivity() : getContext())
                .setTitle("Unsaved changes")
                .setMessage("Continue without saving this project?")
    ...
    private void requestOpen() {
        ...
        Activity activity = currentActivity();
        if (activity == null) {
            Toast.makeText(getContext(), "The file picker is unavailable", Toast.LENGTH_LONG).show();
            return;
        }
    ...
    private void takePersistablePermission(Uri uri, int flags) {
        ...
        try { getContext().getContentResolver().takePersistableUriPermission(uri, allowed); }
        catch (SecurityException ignored) { }
    }

    private void openFromUri(Uri uri) {
        try (InputStream input = getContext().getContentResolver().openInputStream(uri)) {
            if (input == null) throw new IOException("File is unavailable");
            ...
            persistSession();
            saveRecoveryNow();
            Toast.makeText(getContext(), "Project opened", Toast.LENGTH_SHORT).show();
        } catch (IOException error) {
            Toast.makeText(getContext(), "Unable to open this project", Toast.LENGTH_LONG).show();
        }
    }
    ...
    private void saveToUri(Uri uri) {
        ...
        try (OutputStream output = getContext().getContentResolver().openOutputStream(uri, "wt")) {
            if (output == null) throw new IOException("File is unavailable");
            ...
            Toast.makeText(getContext(), "Unable to save this project", Toast.LENGTH_LONG).show();
        }
    }
    ...
    private String displayName(Uri uri) {
        ...
        return segment == null ? "Project.aink" : segment;
    }

    @Override public void onNewIntent(Intent intent) {
        // No plugin-specific intents are consumed.
    }
    ...
    private void refreshStatus() {
        String marker = dirty ? " • modified" : "";
        String layer = canvas.getActiveLayer() == AnimationProject.LAYER_SKETCH ? "Sketch" : "Final";
        String hidden = canvas.isLayerVisible(canvas.getActiveLayer()) ? "" : " (hidden)";
        status.setText(String.format(Locale.getDefault(), "%s%s    Frame %d / %d    %s%s",
    ...
    private void saveRecoveryNow() {
        ...
        try { ProjectStore.saveRecovery(getContext(), project); }
